...
export const options = {
  ...
  plugins: {
    ...
    title: {
      display: true,
      text: "Thống kê sản phẩm bán chậm",
    },
  },
};
...
function TKSPBanCham() {
  const [datatest, setData] = useState();
  const [labesSet, setLabesSet] = useState();

  useEffect(() => {
    (async () => {
      const product = await axios.get("/api/admin/static-bancham");
      if (product.data) {
  ...
  return (
    <div className="container-home container">
      <div className="title-primary pb-4">Thống kê sản phẩm bán chậm</div>
